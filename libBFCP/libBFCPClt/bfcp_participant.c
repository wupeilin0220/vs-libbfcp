...
unsigned __stdcall recv_thread(void* func);
#else
...
unsigned __stdcall recv_thread(void* func) 
{
	int(*received_msg)(bfcp_received_message * recv_msg) = (int(*)(bfcp_received_message*))func;
	...
	for (;;) {
		...
		error = select(0, &rset, NULL, NULL, NULL);
